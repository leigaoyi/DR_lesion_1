...
#from PSPNet import get_pp_unet
#from model_attention import get_att_unet
...
from params import parse_arguments_pred, default_params
...
def predict(**params):
    ...
    if model_name == 'SC_UNet': #spatial-channel attention UNet
        model = get_SCUNet(patch_size, patch_size, channels, n_classes)
        
    if model_name == 'SE_UNet': # bottleneck attention UNet
        model = get_SE_UNet(patch_size, patch_size, channels, n_classes)
        
    if model_name == 'CBAM_UNet':
        model = get_CBAM_UNet(patch_size, patch_size, channels, n_classes)
        
    if model_name == 'CBAM_UNet_V2':
        model = get_CBAM_UNet_v2(patch_size, patch_size, channels, n_classes)
    ...
    model.compile(optimizer=Adam(lr=lr), loss=gen_dice_multilabel, metrics=metrics)
